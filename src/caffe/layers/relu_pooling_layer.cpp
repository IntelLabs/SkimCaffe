...
void ReLUPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
...
#pragma omp parallel for collapse(2)
      for (int n = 0; n < num; ++n) {
        for (int c = 0; c < this->channels_; ++c) {
          // compute offset
          const Dtype *bottom_data_cur = bottom_data + bottom[0]->offset(0, 1)*(this->channels_*n + c);
          Dtype *top_data_cur = top_data + top[0]->offset(0, 1)*(this->channels_*n + c);
          int *mask_cur = mask + top[0]->offset(0, 1)*(this->channels_*n + c);
          Dtype *top_mask_cur = top_mask + top[0]->offset(0, 1)*(this->channels_*n + c);
          ...
          if (this->stride_h_ == this->stride_w_ && this->kernel_h_ == this->kernel_w_ && this->pad_h_ == this->pad_w_ && this->height_ == this->width_) {
            if (3 == this->kernel_w_) {
              if (2 == this->stride_h_ && 3 == this->kernel_w_ && 0 == this->pad_h_) {
                if (112 == this->height_) {
                  pool_<Dtype, 2, 2, 3, 3, 0, 0, 112, 112>(bottom_data_cur, top_data_cur, mask_cur, Dtype(0));
                  continue;
                }
                else if (56 == this->height_) {
                  pool_<Dtype, 2, 2, 3, 3, 0, 0, 56, 56>(bottom_data_cur, top_data_cur, mask_cur, Dtype(0));
                  continue;
                }
                else if (28 == this->height_) {
                  pool_<Dtype, 2, 2, 3, 3, 0, 0, 28, 28>(bottom_data_cur, top_data_cur, mask_cur, Dtype(0));
                  continue;
                }
                else if (14 == this->height_) {
                  pool_<Dtype, 2, 2, 3, 3, 0, 0, 14, 14>(bottom_data_cur, top_data_cur, mask_cur, Dtype(0));
                  continue;
                }
              }
              else if (1 == this->stride_h_ && 1 == this->pad_h_) {
                if (28 == this->height_) {
                  pool_<Dtype, 1, 1, 3, 3, 1, 1, 28, 28>(bottom_data_cur, top_data_cur, mask_cur, Dtype(0));
                  continue;
                }
                else if (14 == this->height_) {
                  pool_<Dtype, 1, 1, 3, 3, 1, 1, 14, 14>(bottom_data_cur, top_data_cur, mask_cur, Dtype(0));
                  continue;
                }
                else if (7 == this->height_) {
                  pool_<Dtype, 1, 1, 3, 3, 1, 1, 7, 7>(bottom_data_cur, top_data_cur, mask_cur, Dtype(0));
                  continue;
                }
              }
            }
