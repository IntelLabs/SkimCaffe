      ...
      for (int n = 0; n < this->num_; ++n) {
        ...
        if (this->param_propagate_down_[0]) {
          ...
          caffe_cpu_gemm<float>(CblasTrans, CblasTrans,
              tile_h_in*tile_w_in, this->conv_in_channels_*ntiles_h*ntiles_w, tile_h_in*tile_w_in,
          ...
          for (int j = 0; j < tile_h_in*tile_w_in; ++j) {
            for (int g = 0; g < this->group_; ++g) {
              caffe_cpu_gemm<float>(CblasNoTrans, CblasTrans,
                  this->conv_out_channels_/this->group_, this->conv_in_channels_/this->group_, ntiles_h*ntiles_w,
                  (float)1,
