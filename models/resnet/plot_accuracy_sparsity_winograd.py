...
winograd_re = re.compile('Winograd Old Sparsity %:')
  ...
  with open(f + "/train.info") as logfile:
    for line in logfile:
      ...
      if mode == 0:
        ...
        if m:
          cur_accuracy = float(m.group(1))
          mode = 1
      elif mode == 1:
        m = accuracy_top5_re.search(line)
        if m:
          cur_accuracy_top5 = float(m.group(1))
          mode = 2
      elif mode == 2:
        m = iteration_re.search(line)
        if m:
          cur_iteration = int(m.group(1))
          mode = 3
      elif mode == 3:
        ...
        if m:
          ...
          accuracy_top5.append(cur_accuracy_top5)
          iteration.append(cur_iteration)
          for i in range(len(layer_names)):
